...
_my2vw = {
    ...
    "YEAR"             : ("SMALLINT"                    , "<COLNAME>", "<VALUE>"  ), # MY: 1901:2155, 0000
    "NCHAR"            : ("NCHAR(<PRECISION>)"          , "<COLNAME>", "'<VALUE>'"),
    "CHAR"             : ("CHAR(<PRECISION>)"           , "<COLNAME>", "'<VALUE>'"),
...
_my2ii = {
    ...
    "VARCHAR"          : ("VARCHAR(<PRECISION>)"        , "<COLNAME>", "'<VALUE>'"),
    "TEXT"             : ("VARCHAR(16000)"              , "<COLNAME>", "'<VALUE>'"), # MS: 2^31 Chars
    "LONGTEXT"         : ("LONG VARCHAR"                , "<COLNAME>", "'<VALUE>'"), # MS: 2^31 Chars
    "LONGBLOB"         : ("LONG BYTE"                   , "<COLNAME>", "'<VALUE>'"), # MS: 2^31 Chars
    "BINARY"           : ("BYTE(<PRECISION>)"           , "<COLNAME>", "<VALUE>"  ), # MS: [1:8000] (Fixed size)
    "VARBINARY"        : ("VARBYTE(<PRECISION>)"        , "<COLNAME>", "<VALUE>"  ), # MS: [1:8000] (Var size)
}
...
_td2vw = {
    "INT"              : ("INTEGER"                     , "<COLNAME>", "<VALUE>"  ), # Signed 4 bytes integer (-2^31; 2^31 - 1 (2,147,483,647))
    "N/A"              : ("INTEGER_N/A"                     , "<COLNAME>", "<VALUE>"  ), # Signed 4 bytes integer (-2^31; 2^31 - 1 (2,147,483,647))
    "TD_ANYTYPE"       : ("INTEGER_TD_ANYTYPE"                     , "<COLNAME>", "<VALUE>"  ), # Signed 4 bytes integer (-2^31; 2^31 - 1 (2,147,483,647))
    "UDT TYPE"         : ("INTEGER_UDT TYPE"                     , "<COLNAME>", "<VALUE>"  ), # Signed 4 bytes integer (-2^31; 2^31 - 1 (2,147,483,647))
    "INTEGER_UDT"      : ("INTEGER_UDT X"                     , "<COLNAME>", "<VALUE>"  ), # Signed 4 bytes integer (-2^31; 2^31 - 1 (2,147,483,647))
    "INTEGER"          : ("INTEGER"                     , "<COLNAME>", "<VALUE>"  ), # Signed 4 bytes integer (-2^31; 2^31 - 1 (2,147,483,647))
    "NUMERIC"          : ("DECIMAL(<PRECISION>,<SCALE>)", "<COLNAME>", "<VALUE>"  ), # MSSQL: -10^38 +1 through 10^38 –1. 
    "NUMBER"           : ("DECIMAL(<PRECISION>,<SCALE>)", "<COLNAME>", "<VALUE>"  ), # MSSQL: -10^38 +1 through 10^38 –1. 
    "DECIMAL"          : ("DECIMAL(<PRECISION>,<SCALE>)", "<COLNAME>", "<VALUE>"  ),
    "TINYINT"          : ("SMALLINT"                    , "<COLNAME>", "<VALUE>"  ), # SQL: [O:255] - VW(TINYINT): -128:+127
    ...
    "TIME"             : ("TIME"                        , "<COLNAME>", "'<VALUE>'"),
    "INTERVAL YEAR TO MONTH"     : ("INTERVAL YEAR TO MONTH"                        , "<COLNAME>", "'<VALUE>'"),
    "INTERVAL DAY TO HOUR"     : ("INTERVAL DAY TO SECOND"                        , "<COLNAME>", "'<VALUE>'"),
    "INTERVAL DAY TO MINUTE"     : ("INTERVAL DAY TO SECOND"                        , "<COLNAME>", "'<VALUE>'"),
    "INTERVAL DAY TO SECOND"     : ("INTERVAL DAY TO SECOND"                        , "<COLNAME>", "'<VALUE>'"),
    "INTERVAL HOUR TO MINUTE"     : ("INTERVAL DAY TO SECOND"                        , "<COLNAME>", "'<VALUE>'"),
    "INTERVAL HOUR TO SECOND"     : ("INTERVAL DAY TO SECOND"                        , "<COLNAME>", "'<VALUE>'"),
    "INTERVAL MINUTE TO SECOND"     : ("INTERVAL DAY TO SECOND"                        , "<COLNAME>", "'<VALUE>'"),
    "INTERVAL YEAR"     : ("INTERVAL DAY TO SECOND"                        , "<COLNAME>", "'<VALUE>'"),
    "INTERVAL MONTH"     : ("INTERVAL DAY TO SECOND"                        , "<COLNAME>", "'<VALUE>'"),
    "INTERVAL DAY"     : ("INTERVAL DAY TO SECOND"                        , "<COLNAME>", "'<VALUE>'"),
    "INTERVAL HOUR"     : ("INTERVAL DAY TO SECOND"                        , "<COLNAME>", "'<VALUE>'"),
    "INTERVAL MINUTE"     : ("INTERVAL DAY TO SECOND"                        , "<COLNAME>", "'<VALUE>'"),
    "INTERVAL SECOND"     : ("INTERVAL DAY TO SECOND"                        , "<COLNAME>", "'<VALUE>'"),
    "DATE"             : ("ANSIDATE"                    , "<COLNAME>", "'<VALUE>'"), # MY: 1999-01-08
    "CHAR"             : ("CHAR(<PRECISION>)"           , "<COLNAME>", "'<VALUE>'"),
    "CHARACTER"        : ("CHAR(<PRECISION>)"           , "<COLNAME>", "'<VALUE>'"),
    "VARCHAR"          : ("VARCHAR(<PRECISION>)"        , "<COLNAME>", "'<VALUE>'"),
    ...
    "VARBINARY"        : ("VARBINARY(<PRECISION>)"      , "<COLNAME>", "<VALUE>"  ), # MS: [1:8000] (Var size)
    "VARBYTE"          : ("VARBYTE(<PRECISION>)"      , "<COLNAME>", "<VALUE>"  ), # MS: [1:8000] (Var size)
    "PERIOD(DATE)"     : ("TIMESTAMP"                   , "<COLNAME>", "'<VALUE>'"),
    "PERIOD(TIMESTAMP)": ("TIMESTAMP"                   , "<COLNAME>", "'<VALUE>'"),
    "PERIOD(TIMESTAMP WITH TIME ZONE)": ("TIMESTAMP WITH TIME ZONE"                   , "<COLNAME>", "'<VALUE>'"),
    "PERIOD(TIME WITH TIME ZONE)"     : ("TIMESTAMP"                   , "<COLNAME>", "'<VALUE>'"),
    "PERIOD(TIME)"     : ("TIMESTAMP"                   , "<COLNAME>", "'<VALUE>'"),
    "BLOB"             : ("NVARCHAR(<PRECISION>)"       , "CAST(<COLNAME> AS VARCHAR(64000))", "'<VALUE>'"),  # OR: 4GB-1*db_block_size
    "CLOB"             : ("NVARCHAR(<PRECISION>)"       , "CAST(<COLNAME> AS VARCHAR(64000))", "'<VALUE>'"),  # OR: 4GB-1*db_block_size
    "JSON"             : ("NVARCHAR(<PRECISION>)"       , "CAST(<COLNAME> AS VARCHAR(64000))", "'<VALUE>'"),  # OR: 4GB-1*db_block_size
    "XML"              : ("VARCHAR(30000)"               , "CAST(<COLNAME> AS VARCHAR(64000))", "'<VALUE>'") # MSSQL : 2 GB
}
...
_unsupported_types_mapping = {
    "mssql" : {
        ...
        "avalanche": ['image', 'hierarchyid', 'geometry', 'geography', 'varbinary', 'binary', 'xml'],
        "actianx": ['hierarchyid', 'geometry', 'geography''xml'],
        "ingres": [ 'hierarchyid', 'geometry', 'geography', 'xml'],
        "vectorh": ['image', 'hierarchyid', 'geometry', 'geography', 'varbinary', 'binary', 'xml']
    },
    "mysql" : {
        "vector": ['image', 'hierarchyid', 'geometry', 'geography', 'varbinary', 'binary', 'xml'],
        "avalanche": ['image', 'hierarchyid', 'geometry', 'geography', 'varbinary', 'binary', 'xml'],
    ...
    "postgres" : {
        "vector": ['BYTEA', 'ARRAY', 'BIT VARYING', 'VARBINARY', 'varbinary', 'image', 'xml'],
        "avalanche": ['BYTEA', 'ARRAY', 'BIT VARYING', 'VARBINARY', 'varbinary', 'image', 'xml'],
        "actianx": ['hierarchyid',  'ARRAY', 'geometry', 'geography''xml'],
        "ingres": [ 'hierarchyid',  'ARRAY', 'geometry', 'geography', 'xml'],
        "vectorh": ['BYTEA', 'ARRAY', 'BIT VARYING', 'VARBINARY', 'varbinary', 'image', 'xml']
    },
    "netezza" : {
        "vector": ['BYTEA', 'ARRAY', 'BIT VARYING', 'VARBINARY', 'varbinary', 'image', 'xml'],
        "avalanche": ['BYTEA', 'ARRAY', 'BIT VARYING', 'VARBINARY', 'varbinary', 'image', 'xml'],
    ...
    "zen" : {
        ...
        "avalanche": ['RAW', 'CLOB', 'BLOB', 'BYTE', 'VARBYTE', 'BINARY' ],
        "vectorh": ['RAW', 'CLOB', 'BLOB', 'BYTE', 'VARBYTE', 'BINARY' ],
        "ingres": ['RAW', 'CLOB' ],
        "actianx": ['RAW', 'CLOB']
    },
    "oracle" : {
